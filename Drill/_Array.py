...
# 삭제
# 결측값이 존재하는 모든 행(관측값) 삭제
df.dropna(axis=0)
# 결측값이 존재하는 모든 컬럼(변수) 삭제
df.dropna(axis=1)
# 지정된 컬럼에 결측값이 존재하는 행(관측값)만 삭제
df.dropna(subset=['city', 'pop'], axis=0)
# thresh 옵션은 정상값이 지정 갯수 미만인 컬럼 삭제
...
df.area.fillna(mean_age)
# S.where(), DF.where(): 조건이 true면 원래값 유지, false면 명시된 값으로 대체
df.where(pd.notnull(df), df.mean(), axis='columns')

# 최빈값 대체
# value_counts(): S의 unique value로 그룹핑해서 count하는 함수(groupby()에선 그룹 중복으로 피함)
most_freq = df['pop'].value_counts(dropna=True).idxmax()
df['pop'].fillna(most_freq)

# 이웃값 대체('ffill', 'bfill')
df['pop'].fillna(method='ffill', inplace=False)

# 컬럼별 대체
df.fillna({'pop': most_freq, 'area': mean_age})
...
# >>> to_frame(): 시리즈를 데이터베이스로 변환
ar = pd.Series(['a', 'b', 'c', 'd', 'e'])
test = ar.to_frame()
test

# >>> isinstance(): 변수의 데이터프레임 여부 확인
...
# # 배열 인덱싱
mdf['object']           # 컬럼 0 레벨
mdf[('object', 'pop')]  # 튜플 인수

# # loc 인덱서
mdf.loc[('2000', 'A_Seoul')]
mdf.loc['2000', ('number', 'area')]
# slice(None): 특정 레벨의 모든 인덱스를 선택(튜플 내에서의 ':'사용제약 대체)
mdf.loc['2000', (slice(None), 'pop')]
...
# # 피벗테이블 ----------
# 행인덱스와 컬럼인덱스의 조건을 만족하는 데이터가 2개 이상인 경우는 에러가 발생
...
# # 형 변환
df['area'].replace(np.nan, 0).astype('int64')

# # 천단위 콤마 삭제
df['pop'].str.replace(',', '').replace(np.nan, 0).astype('int64')

# # 천단위 콤마 추가
df['area'].apply(lambda x: format(x, ','))

# # 소수점 자리수 변경
df['area'].apply(lambda x: '{:.2f}'.format(x))
...
# # 상대적 크기 차이 삭제
# 컬럼데이터에서 컬럼최솟값을 뺀 값을 분자로 하고, 컬럼최댓값-컬럼최솟값을 분모로 산출(0~1)
...
# # pd.cut(): 수치형 컬럼(변수)을 동일한 길이(경계값 기준 등)으로 분할
...
# # pd.qcut(): 수치형 컬럼(변수)을 동일한 갯수로 분할
...
# # 더미 컬럼 생성: 분할 라벨별 컬럼 생성(속하면1, 속하지 않으면0)
...
# apply()와 달리 매서드안에서는 컬럼명을 사용할 수 없고, 연산 불가능한 컬럼은 자동 필터링됨
