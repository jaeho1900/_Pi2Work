...
plt.hist(gdf2['이동시간(분)'], bins=20, color='r')
plt.xlabel('이동시간(분)')
plt.ylabel('빈도수(건)')
...
# 이동시간이 길었던 작업들
long_travel_10 = gdf2[gdf4['이동시간(분)'] > gdf2['이동시간(분)'].quantile(0.90)]
...
work_avg = gdf4['작업시간(분)'].mean()     # 123.5
travel_avg = gdf4['이동시간(분)'].mean()   # 73.7
