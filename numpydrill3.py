# >>> Broadcasting: 퍼지게한다, 배열의 형상을 확장시킨다
# 스칼라값, 한쪽배열크기1, 동일차원, 동일축길이인 경우에 가능!!
import numpy as np
ar30 = np.arange(1, 31).reshape(5, 6)
ar30

# >>> Broadcasting 산술연산
# ndarray에 스칼라(상수) 3 더하기
...
# 스칼라 값은 넘파이 내부에서 np.array([3,3,3,3,3])로 확장되어 계산된 것임
a + 3

# 일반적으로 형상이 다르면 오류가 난다
a + np.array([1, 2])

# 열 갯수가 같으면 행으로 자동 확장됨
a = np.ones((4, 4))
b = np.arange(4)
a
b
