...
df4['광역시도'].value_counts(dropna=True)
df4['행정구역'].value_counts(dropna=True)
...
user_area = pd.DataFrame(df4['광역시도'].unique(), columns=['광역시도'])
std_area_df.merge(user_area, left_on=['표준광역시도'], right_on=['광역시도'], how='outer')
...
df4['광역시도'] = df4['광역시도'].apply(lambda v: 광역시도_aliases.get(v, v))

# ----- 광역시도 + 행정구역 합치기 -----

df4['단축명칭'] = df4['행정구역'].apply(lambda x: x[:-1])

df4['단축명칭'] = df4['단축명칭'].where(~(df4['광역시도'].str.contains('^강원') & df4['행정구역'].str.contains('^고성')), '강원고성')
df4['단축명칭'] = df4['단축명칭'].where(~(df4['광역시도'].str.contains('^경상') & df4['행정구역'].str.contains('^고성')), '경남고성')

condition = df4['행정구역'].apply(lambda x: len(x) > 2)

df4['단축명칭'] = df4['단축명칭'].where(~(df4['광역시도'].str.contains('^(?!세종).*시$') & condition), \
                     df4['광역시도'].str[:2] + df4['행정구역'].str[:-1])
df4['단축명칭'] = df4['단축명칭'].where(~(df4['광역시도'].str.contains('^(?!세종).*시$') & ~condition), \
                     df4['광역시도'].str[:2] + df4['행정구역'].str[:])
df4['단축명칭'].unique()
len(df4['단축명칭'].unique())

df4.drop_duplicates(subset=['단축명칭'], keep='last')
